...
def funcion_modificar():
    conexion=mysql.connector.connect(user='root', host='localhost', database='refaccionaria', port='3306') 
    con=conexion.cursor()
    mod_select=tabla.selection()
    if mod_select:
        primer_sel=mod_select[0]
        datos_modi=tabla.item(primer_sel,"values")[0]
        veninv.destroy()
        Modificar_Interfaz.main((datos_modi, ), 1)
    else:
        messagebox.showerror("Atención", "No se selecciono nada")#si no se selecciono algo de la tabla   
...
def main(idar,nombre,precio,descripcion,modelo,anio,marca):
    ...
    consulobj.grid(column=0, row=15, padx=(120,0))
    
    modificar = Button(veninv, width=20, font=('Arial', 12, 'bold'), text='Modificar', bg='blue', bd=5, command=funcion_modificar)
    modificar.grid(column=1, row=15)
